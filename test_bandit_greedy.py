from bandit_greedy import BanditGreedy
import numpy as np


def test_greedy_bandit():

    greedy_bandit = BanditGreedy(3)
    ...
    assert(greedy_bandit.get_means().argmax() == 2)
    assert (choice_counts[2] > choice_counts[1] and choice_counts[2] > choice_counts[0])
    assert(np.all(np.isclose(greedy_bandit.get_means(), means, 0.1)))
